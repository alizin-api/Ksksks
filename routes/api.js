...
var kuhong = db.get('ojan');
	...
	console.log('Bem vindo ao ALIZIN API!') 
}

var criador = `ALIZINDEV`
var key = 'ALIZINDEV' 

var chaveapi = 'Key-do-alizin'



const listkey = ["alizin-rest"];
...
fetch(encodeURI(`https://alizindev-api.onrender.com/api/playaudio2?quero=${quero}&apikey=${chaveapi}`))
...
router.get('/tiktok', async(req, res, next) => {
  ...
  if(!apikey) return res.json(precisos.digitarapikey)
  if(listkey.includes(apikey)){
fetch(encodeURI(`https://alizindev-api.onrender.com/api/tiktok?url=${url}&apikey=${chaveapi}`))
        .then(response => response.json())
        .then(dataf => {
...
router.get('/horror', async (req, res, next) => {
   ...
   if(!apikey) return res.json(precisos.digitarapikey)
	if(listkey.includes(apikey)){    
      deuz = 'https://alizindev-api.onrender.com/api/horror?texto=' + texto + '&apikey=' + chaveapi + ''
	  data = await fetch(deuz).then(v => v.buffer())   
